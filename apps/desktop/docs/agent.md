...
- **The face has its own clock, and it is the visible one.** The cadence above decides when the agent runs; two other windows decide whether anyone can see that it did. A turn earns a face for `HOSTILEPET_PET_LINE_TTL_MS` (default 60 s) and that expiry is what repaints the pet back to idle — a mood is dropped earlier only when the site it was about stops being looked at, which is `HOSTILEPET_PRESENCE_STALE_MS` (default 15 s, with the accrued time itself forgotten at `HOSTILEPET_PAGE_STALE_MS`, default 90 s). Shortening the cadence alone therefore leaves the pet looking stuck: it reacts sooner and then holds the face for the full minute.
- **Demo pacing is one switch, and it is badged.** `HOSTILEPET_DEMO_FAST=1` lowers every one of those clocks together (1.5 s settle, 2.5 s max wait, 5 s face, 3 s presence, 1 s observe throttle) so a whole cycle fits in about five seconds, and Settings reports the run as DEMO while any threshold or clock is lowered (`docs/hackathon.md` §4). It moves clocks only: no threshold that decides *whether* the pet reacts changes, so a demo still shows the real sensor and the real ladder. Setting any clock explicitly beats the preset. Measured against the real chain, the reaction lands about 2.5 s after a page opens — the max wait, not the settle window, is the binding one while the page keeps ticking — and the face is gone within 5 s of the tab going quiet.
