...
The desktop-only scaffold contains Electron main/preload/React renderer, a tray, draggable transparent placeholder pet, show/hide controls, a settings window and explicit Quit. Shell IPC is validated; windows deny permissions and unrequested navigation. Original CSS geometry is labelled as placeholder art in Settings. Nothing is previewed on demand any more: the pet's bubble appears only when a model wrote a line (ADR 0011), and the floating pet has no label card underneath it.
...
The **event → agent → pet chain** exists as the first vertical slice of behaviour. `src/main/events/` classifies a host against `packs/site-catalog.json`, accrues qualifying time per page (`site-tracker`), and writes redacted observations to a capped ring buffer (`event-log`, ADR 0008). `packages/agent` turns a context package into one line and one action — provider adapter, deterministic validator, one retry, and silence when no model produced a line (ADR 0011). With no model configured the pet has nothing to say and Settings names why; `HOSTILEPET_PROVIDER=openai` swaps in one `POST /chat/completions` whose key `src/main/agent/api-key.ts` reads from `.env` first and the login Keychain second (ADR 0010) and whose prompt `src/main/agent/persona.ts` loads from `prompts/persona.md`. `src/main/env-file.ts` reads `.env` at startup, before the first `HOSTILEPET_*` read, so a packaged build sees the same file a `pnpm dev` run does. `src/main/agent/turn-runner.ts` owns the 30–60 s cadence and the escalation level; `src/main/pet/presenter.ts` is the only place a line becomes visible, and it always carries its provenance. `apps/desktop/scripts/simulate-site.cjs` drives the whole chain without a browser extension.

What does not exist: a rule engine, a pack runtime, capability grants, a persisted `state.json`, and the Live2D character. The model call exists but has only ever been exercised against a stub transport — it has never reached the API, so the pinned model is unproven (`docs/agent.md` §7). The mock handler is a stand-in: it proves the protocol, never the policy. There is no stand-in for the language — with no model running, the pet has nothing to say and says nothing (ADR 0011). Position survives window hide/show but is not yet persisted across app restarts. The remaining layout below is a target; `apps/desktop/src/main/{bridge,events,agent,pet,focus}`, `apps/desktop/packs` and `packages/{contracts,agent}` are what exists.
...
apps/
  desktop/
    ...
    src/main/agent/         # Turn cadence, escalation level, context assembly, API key, persona
    src/main/env-file.ts    # Reads .env at startup (dev and packaged), ADR 0010
...
packages/
  ...
  agent/                    # Provider adapters (no-model, OpenAI), prompt assembly, validator
  ...
  tone/                     # Tone schema, validator, eval scenarios
...
- **Secrets:** the login Keychain, plus the model key in the git-ignored `.env` (ADR 0010) — main-process only, never logged, never persisted. The only other exception is the extension-side bridge token documented in ADR 0001. A real key in a tracked file, a log or an example stays a defect; examples contain placeholders only.
...
5. **Agent.** Tool loop, structured output, validator, honest silence when no model answers (ADR 0011), then the agent-drafted pack flow.
...
- [ ] Required headless checks pass; no new lint warnings; no secret in the diff.
- [ ] Every line on screen passed the validator; a turn that got no line from the model showed nothing at all and is stamped `fallback` in the log.
